...
app.get("/", async (request, reply) => {
  return {
    intro:
      "Welcome to the unofficial multi provider resolver and eporner api currently the ONLY All-In-One solution aswell as additional Eporner resolver.",
    documentation: "soon",
...
app.get("/vidsrc", async (request, reply) => {
  try {
    ...
    if (!provider) {
      return reply
        .status(400)
        .send({ message: "The 'provider' query is required" });
    }
      ...
      try {
        ...
        if (seasonNumber && episodeNumber) {
          ...
          if (!response) {
            return reply
              .status(404)
              .send({ status: 404, return: "Sources not found." });
          } else {
            const data = {
              res,
            };
    ...
    const fetchEporner = async (
      ...
      if (search) {
        const getResults = await getSearchResults(
          search,
          per_page,
          page,
          thumbsize,
